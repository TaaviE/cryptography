...
use cryptography_x509::oid::{
    BASIC_CONSTRAINTS_OID, EC_SECP256R1, EC_SECP384R1, EC_SECP521R1,
    EKU_CLIENT_AUTH_OID, EKU_EMAIL_PROTECTION_OID, EKU_SERVER_AUTH_OID,
...
impl<'a, B: CryptoOps> Policy<'a, B> {
    ...
    pub(crate) fn valid_issuer<'chain>(
        ...
        if matches!(
            ...
            let rsa_key: pkcs1::RsaPublicKey<'_> = pkcs1::RsaPublicKey::from_der(
                issuer_spki.subject_public_key.as_bytes(),
            )
            .map_err(|e| {
                ValidationError::new(ValidationErrorKind::Other(format!(
                    "subject public RSA key could not be parsed: {}",
                    e
                )))
            })?;
